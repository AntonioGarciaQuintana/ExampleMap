...
@Component({
    templateUrl: 'registrophotos.component.html'
})
export class RegistroPhotoComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        this.dropifyElement2 = $('.dropify2').dropify({
            error: {
                'fileSize': 'The file size is too big ({{ value }} max).',
                'maxWidth': 'The image width is too big ({{ value }}px max).',
                'maxHeight': 'The image height is too big ({{ value }}px max).',
    ...
    reset2() {
        const drEvent = $('.dropify2').dropify();
        const dropify = drEvent.data('dropify2');
        dropify.resetPreview();
        dropify.clearElement();
    }
